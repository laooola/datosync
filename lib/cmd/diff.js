...
function normalizeModels(models) {
  ...
  models.forEach(model => {
    ...
    if (model.titleField) {
      ...
      const titleField = model.fields.find(field => field.id === model.titleField)
...
function diffModel(modelA, modelB) {
  ...
  return {
    ...
    model: {
      ...
      label: (modelA && modelA.name) || (modelB && modelB.name),
...
function printSuccess(diffs) {
  ...
  diffs.forEach(({ isEqual, model, fields }) => {
    if (isEqual) {
      logger.log(Output.pass(), Output.dim(`model/${model.apiKey}`), Output.title(model.label))
    } else {
      logger.log(Output.fail(), Output.dim(`model/${model.apiKey}`), Output.title(model.label))
      if (!model.isEqual) {
        logger.log(Output.diff(`model/${model.apiKey} > ${model.label}`))
        printDiff(model.diff)
      }
      fields.forEach(field => {
        if (!field.isEqual) {
          logger.log(Output.diff(`model/${model.apiKey}/field/${field.apiKey} > ${field.label}`))
  ...
  logger.log()
  logger.log(Output.title('Models:'), Output.text(`\t${diffs.length} total`))
