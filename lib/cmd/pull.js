...
async function pullModelsAndFields(token) {
  ...
  const fields = await Promise.all(models.map(model => client.fields.all(model.id)))
...
function printPullError(err, token) {
  logger.log(Output.fail(), Output.title(`Cannot pull models and fields with token "${token}"`))
  logger.log(
    Output.text(
      JSON.stringify(
        {
