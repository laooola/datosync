...
describe('pull', () => {
  ...
  it('prints success message', async () => {
    ...
    expect(log.mock.calls[0]).toEqual([Output.pass(), Output.title('Pulled all models and fields')])
    expect(log.mock.calls[1]).toEqual([Output.title('Models:'), Output.text(`\t2 total`)])
    expect(log.mock.calls[2]).toEqual([Output.title('Fields:'), Output.text(`\t4 total`)])
    expect(log.mock.calls[3]).toEqual([Output.text('Results written to "filename.json".')])
  })
